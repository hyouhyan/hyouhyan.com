...
src/
  ...
  components/
    ...
    Career.astro      ← タイムライン（src/data/career.ts）
    Skills.astro      ← カテゴリ別スキルグリッド。データは src/data/skills.ts
    Works.astro       ← ホバーオーバーレイカード（src/data/works.ts）＋ h2「Publications」サブセクション（src/data/publications.ts）。publications が空なら非表示
...
5. **Work** `#work` … 制作物カード（3件）＋ h2「Publications」サブセクション（論文・学会発表。`src/data/publications.ts` に1件ずつ追加。空なら非表示、ナビ（h1）は増やさない方針）
