...
src/
  ...
  components/
    ...
    Hobby.astro       ← 趣味セクション。趣味カード（src/data/hobby.ts）＋ h2「推し」サブセクションの推しカード（src/data/oshi.ts）を1コンポーネントに内包
  ...
  data/
    ...
    oshi.ts           ← OshiItem[]（name / description / category? / img? / url?）。Hobby.astro の h2「推し」サブセクションで描画。プレースホルダー1件。img 未設定ならハートアイコン表示、url があればカード全体がリンク化。中身はユーザが後で記入
...
6. **Hobby** `#hobby` … 趣味カード＋ h2「推し」サブセクション（推しカード）。**中身はプレースホルダー（趣味: ダーツ・自宅サーバ運用／推し: 1件、いずれも要記入）**
